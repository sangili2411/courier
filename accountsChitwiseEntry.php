                                    ...
                                    <div class="row">
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" id="group-select" onchange="groupSelectChanged(this)">
                                                    <option value=""> -- Select Group -- </option>
                                                    <?php
                                                    $userId = $_SESSION['userId']; // decryptAgentId(preg_replace('/[^0-9]/', '', $_GET['userId']), 3);
                                                    $selectGroupSql = "SELECT DISTINCT GROUP_ID, GROUP_NAME FROM v_chit_accounts_details WHERE CREATED_BY_AGENT = " . $userId . " ORDER BY 1";
...
<script>
    ...
    let groupSelectChanged = function(select) {
        let groupId = $(select).val();
        $.ajax({
            url: "dataOperations.php",
            type: "POST",
            data: {
                getChitDatesForSelectedGroup: 1,
                groupId: groupId
            },
    ...
    let generateGrid = function(accountDetailsJson) {
        for (let i = 0; i < accountDetailsJson.length; i++) {
            let htmlString = '<div class="row bdr-btm-orange t-1-5" id=row-"' + i + '">' +
                ...
                '</div>' +
                '<div class="col-sm-2">' +
                '    <label id="member-name-' + i + '" data-account-id="' + accountDetailsJson[i]["ACCOUNT_ID"] + '" data-member-id="' + accountDetailsJson[i]["MEMBER_ID"] + '">' + accountDetailsJson[i]["MEMBER_NAME"] + " / " + accountDetailsJson[i]["MEMBER_MOBILE"] + '</label>' +
                '</div>' +
                '<div class="col-sm-2">' +
                '    <label id="monthly-amount-' + i + '">' + accountDetailsJson[i]["MONTHLY_AMOUNT"] + '</label>' +
                '</div>' +
                '<div class="col-sm-2">' +
                '    <label id="advance-amount-' + i + '">' + nullCheck(accountDetailsJson[i]["PAID_AMOUNT"]) + '</label>' +
                '</div>' +
                '<div class="col-sm-2">' +
                '    <label id="existing-balance-' + i + '">' + nullCheck(accountDetailsJson[i]["MONTHLY_AMOUNT"] - accountDetailsJson[i]["PAID_AMOUNT"]) + '</label>' +
                '</div>' +
                '<div class="col-sm-2">' +
        ...
        noOfMember = accountDetailsJson.length;
    ...
    let generateAmountGrid = function(btn) {
        ...
        if (groupId == "") {
            ...
            $.ajax({
                ...
                type: "POST",
                data: {
                    getChitPaymentDetailsForSelectedGroup: 1,
                    groupId: groupId,
                    month: month
                },
                success: function(response) {
    ...
    let saveFieldValues = function(btn) {
        if (confirm("🚨 Once the changes are saved it can't be updated \nAre you sure to proceed")) {
            ...
            let dataArray = [];
            let canSave = false;
            for (let i = 0; i < noOfMember; i++) {
                let dataObj = {};
                let accountId = $("#member-name-" + i).attr("data-account-id");
                let memberId = $("#member-name-" + i).attr("data-member-id");
                ...
                if (isNaN(parseInt(paidAmount)) && paidAmount != "") {
                    ...
                    dataObj["GROUP_ID"] = $("#group-select").val();
