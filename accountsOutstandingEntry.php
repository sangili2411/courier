...
<script>
    ...
    let convertDate = function(dateStr) {
        ...
        var monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]; // Array of month names
        var formattedDate = year + '-' + monthNames[date.getMonth()]; // Format the date as YYYY-MMM
    ...
    let generateGrid = function(accountDetailsJson) {
        ...
        for (let i = 0; i < accountDetailsJson.length; i++) {
            ...
            let htmlString = '<div class="row bdr-btm-orange t-1-5" id=row-"' + i + '">' +
                ...
                '</div>' +
                '<div class="col-sm-2">' +
                '    <label id="group-name-' + i + '" data-account-id="' + accountDetailsJson[i]["ACCOUNT_ID"] + '" data-group-id="' + accountDetailsJson[i]["GROUP_ID"] + '">' + accountDetailsJson[i]["GROUP_NAME"] + " / " + convertDate(accountDetailsJson[i]["CHIT_MONTH"]) + '</label>' +
                '</div>' +
                '<div class="col-sm-2">' +
    ...
    let generateAmountGrid = function(btn) {
        ...
        if (memberId == "") {
            ...
            $.ajax({
                ...
                type: "POST",
                data: {
                    getChitPaymentDetailsForOutstanding: 1,
                    memberId: memberId
                },
    ...
    let saveFieldValues = function(btn) {
        if (confirm("🚨 Once the changes are saved it can't be updated \nAre you sure to proceed")) {
            $(btn)
                .attr("disabled", true)
                .html('<i class="fa fa-spinner fa-pulse fa-3x fa-fw" style="font-size: small;"></i> <span>Saving</span>');
            let dataArray = [];
            let canSave = false;
            ...
            if (canSave) {
                ...
                $(btn)
                    .attr("disabled", false)
                    .html('<i class="fa fa-floppy-o" aria-hidden="true"></i> Save');;
            }
        }
    };
